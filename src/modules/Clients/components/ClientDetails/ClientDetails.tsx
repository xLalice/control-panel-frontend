...
import { User, MapPin, Mail, Phone, Building2, FileText, Calendar } from "lucide-react";
...
export const ClientDetailsPanel: React.FC<ClientDetailsPanelProps> = ({
  ...
  const initials = clientName.split(' ').map(name => name[0]).join('').toUpperCase().slice(0, 2);

  // Custom header content for the client panel
  const headerContent = (
        ...
        <section>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <p className="text-sm text-gray-900">{new Date(client.createdAt).toLocaleDateString()}</p>
            </div>
            <div className="bg-gray-50 rounded-lg p-3">
              <p className="text-xs font-medium text-gray-600 mb-1">Last Updated</p>
              <p className="text-sm text-gray-900">{new Date(client.updatedAt).toLocaleDateString()}</p>
            </div>
            {client.isActive !== undefined && (
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-xs font-medium text-gray-600 mb-1">Active Status</p>
                <p className="text-sm text-gray-900">{client.isActive ? 'Active' : 'Inactive'}</p>
              </div>
            )}
            {client.convertedFromLeadId && (
              <div className="bg-gray-50 rounded-lg p-3">
                <p className="text-xs font-medium text-gray-600 mb-1">Converted from Lead</p>
                <p className="text-sm text-gray-900 font-mono">{client.convertedFromLeadId}</p>
              </div>
            )}
          </div>
        </section>
